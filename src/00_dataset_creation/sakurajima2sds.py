...
def antelope2sds(startt, endt, inputdirroot, outputdir, net, dbout=None, remove_all=False):
    secondsPerDay = 86400
    if remove_all:
    ...
    while (dayt <= endt):
        ...
        inputdir = os.path.join(inputdirroot, 'dayfiles', '2015', jday)
        #print("inputdir = ",inputdir)
        allfilespattern = os.path.join(inputdir,"*.msd")
        ...
        if len(allfiles)>0:
            ...
            if dbout:
                os.system(f"miniseed2days -U -w '{outputdir}/%Y/%{{net}}/%{{sta}}/%{{chan}}.D/%{{net}}.%{{sta}}.%{{loc}}.%{{chan}}.D.%Y.%j' -S {outputdir} -C {chuckfile} -s {dayepoch} -e {dayendepoch} -d {dboutymd} {allfilesstr}")
            else:    
                os.system(f"miniseed2days -U -w '{outputdir}/%Y/%{{net}}/%{{sta}}/%{{chan}}.D/%{{net}}.%{{sta}}.%{{loc}}.%{{chan}}.D.%Y.%j' -S {outputdir} -C {chuckfile} -s {dayepoch} -e {dayendepoch} {allfilesstr}")
...
inputdirroot =  '14TB/new_from_newton/sakurajima/Seismic_Infrasound/'
outputdir = paths['SDS_DIR']
dbout = os.path.join(paths['DB_DIR'], 'dbSakurajima2015')
startt = obspy.core.UTCDateTime(2015,4,1)
